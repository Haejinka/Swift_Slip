...
if (isset($_GET['attendance_id'])) {
    ...
    $query = "SELECT * FROM archive_attendance WHERE attendance_id = '$attendance_id'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if ($archiveResult) {
            ...
            $deleteQuery = "DELETE FROM archive_attendance WHERE attendance_id = '$attendance_id'";
            $deleteResult = mysqli_query($con, $deleteQuery);

            if ($deleteResult) {
                // Record archived and deleted successfully
                header("Location: archive_attendance.php?");//redirect to the View Attendance
                exit();
            } else {
