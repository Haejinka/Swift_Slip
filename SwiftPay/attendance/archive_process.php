...
if (isset($_GET['attendance_id'])) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $archiveQuery = "INSERT INTO archive_attendance (employee_id,pay_term,date, time_in, time_out, hours_worked ) VALUES ('{$attendanceData['employee_id']}','{$attendanceData['pay_term']}','{$attendanceData['date']}' ,'{$attendanceData['time_in']}', '{$attendanceData['time_out']}', '{$attendanceData['hours_worked']}')";
